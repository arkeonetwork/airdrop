...
var (
	...
	sqlFindEthChains = `select name,rpc_url,lcd_url,snapshot_start_block,snapshot_end_block,coalesce(decimals,0) as decimals from chains where name in ('GNO')` // ('ETH','POLY','GNO')
)
